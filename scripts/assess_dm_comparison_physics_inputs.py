#!/usr/bin/env python3
"""Verify provenance-bound resolved inputs for CDM/SIDM/FDM analyses."""
...
def main() -> int:
    ...
    parser.add_argument("physics_input", type=Path)
    parser.add_argument("output", type=Path)
    args = parser.parse_args()
    assessment = assess_dm_comparison_physics_inputs(
        read_dm_comparison_physics_input(args.physics_input)
    )
    record = assessment.as_dict()
    _write_json_atomic(args.output.expanduser().resolve(), record)
    print(json.dumps(record, indent=2, sort_keys=True))
    return 0 if assessment.ready_for_model_specific_analysis else 2
