...
n512_rk18=${result_root}/torch_convergence/koo_n512_cuda_rk18_0p1myr_s512
comparison_root=${result_root}/torch_convergence
movie_directory=${result_root}/movies
state_directory=${run}/Finalization
failure_marker=${state_directory}/finalization.failed
...
# The 512^3 wave-response analysis uses about 37 GB of resident memory.  Keep
# numerical libraries single-threaded and leave bounded FFT workspace headroom.
ulimit -v 67108864
...
record_failure() {
  local status=$?
  trap - ERR
  record "tripwire finalizer failed with status ${status}; automatic retry disabled"
...
run_resumable_wave_response() {
  local target_run=$1 invocation sample_count
  sample_count=$(python -c \
    'import json,sys; print(int(json.load(open(sys.argv[1]))["saved_3d_states"]))' \
    "${target_run}/fdm_adapter_metadata.json")
  if [[ ! ${sample_count} =~ ^[1-9][0-9]*$ ]]; then
    printf 'invalid saved_3d_states for %s: %s\n' \
      "${target_run}" "${sample_count}" >&2
    return 3
  fi
  for ((invocation = 1; invocation <= sample_count; invocation++)); do
    if (
    ...
    python scripts/analyze_pyul_wave_response.py "${target_run}" \
  ...
  if (
    ...
    printf 'resumable wave response did not complete for %s\n' \
      "${target_run}" >&2
    return 3
  fi
}
...
run_step wave_response \
  run_resumable_wave_response "${run}"
