...
def read_materialized_pure_fdm_dual_soliton_seed(
    path: str | Path,
) -> PureFDMDualSolitonSeed:
    """Re-read one materialized seed and every solver input it names.

    This is deliberately stricter than :func:`load_pure_fdm_dual_soliton_seed`:
    it accepts only the manifest emitted by
    :func:`materialize_pure_fdm_dual_soliton_seed`, checks both materialized
    files and does not infer a missing solver setting.
    """
    ...
    try:
        ...
        raise ValueError(f"cannot read materialized dual-soliton seed: {error}") from error
    expected = {
        "schema_version",
        "status",
        "interpretation",
    ...
    if (
        ...
        or record.get("schema_version") != DUAL_SOLITON_SEED_SCHEMA_VERSION
    ...
    if not isinstance(seed_record, Mapping) or record.get("seed_sha256") != _canonical_sha256(seed_record):
    ...
    if not isinstance(files, Mapping) or set(files) != {
        ...
        raise ValueError("dual-soliton manifest file provenance is invalid")
    expected_contents = {
        "fdm_namelist_fragment": _fdm_namelist_fragment(seed),
        "ic_sink": _ic_sink(seed),
    }
    for name, expected_content in expected_contents.items():
        ...
        if (
            ...
            raise ValueError(f"dual-soliton manifest {name} provenance is invalid")
        ...
        try:
            ...
            raise ValueError(f"cannot read materialized {name}: {error}") from error
        if actual_hash != artifact["sha256"]:
            raise ValueError(f"materialized {name} SHA-256 does not match the seed manifest")
        if actual_content != expected_content:
            raise ValueError(f"materialized {name} content does not match the seed manifest")
    return seed
