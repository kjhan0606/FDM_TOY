...
class DualSolitonRuntimeIdentity:
    """Binding between a materialized seed and one raw V2/V3/V4/V5 solver output."""
...
def _validate_runtime_components(
    ...
    reasons: list[str] = []
    if provenance.source_schema_version not in {2, 3, 4, 5}:
        return ["raw provenance V2, V3, V4, or V5 is required for dual-soliton runtime identity"]
...
def validate_pure_fdm_dual_soliton_runtime_identity(
    *, seed_manifest_path: str | Path, provenance_path: str | Path
    ...
    """Verify the raw V2/V3/V4/V5 output retained the exact materialized two-core state."""
